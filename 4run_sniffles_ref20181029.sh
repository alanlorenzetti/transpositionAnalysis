...
# alorenzetti 20181029
# this script will run ngmlr, sniffles
# to find structural variants on ref genome

# apparently nanosv is not suitable to our purpose
# so i put a switch to prevent analyzing using it by default
...
inputdir=20180920_rotate
fastqdir=20180817_porechop
outputdir=20181029_sniffles_ref
...
# ncbi ftp is not working here bloody hell #&%#%#&*$&
        cp ~/$spp".fa" $miscdir/$spp".fa"
#        curl $url 2> /dev/null | zcat | sed 's/\(>.*\.[0-9]\) .*$/\1/' > $miscdir/$spp".fa"
...
# ncbi ftp is not working here bloody hell #&%#%#&*$&
        cp ~/$spp".gff" $miscdir/$spp".gff"
#        curl ${url/.fna.gz/.gff.gz} 2> /dev/null | zcat > $miscdir/$spp".gff"
        ...
        if [[ ! -f ${miscdir}/${spp}-ISSaga-checked.gff3 ]] ; then
                cp /home/alorenzetti/lsm-hfq/${spp}/misc/${spp}-ISSaga-checked.gff3 $miscdir

                # parsing is annotation gff file
                sed '/^#/d' $miscdir/$spp"-ISSaga-checked.gff3" | grep -v "\-p" | \
                awk -v FS="\t" -v OFS="\t" '{if($3 == "mobile_element"){print}}' > $miscdir/$spp"_ISSaga-checked.gff"
        ...
        for i in $barcodes ; do
                ...
                while IFS="	" read qseqid qstart qend sseqid sstart send score evalue pident ; do
                                ...
                                if [[ "$req1" == 1 ]] && [[ "$req2" == 1 ]] && [[ "$req3" == 1 ]]; then
                                        echo -e "$insertionacc\tsniffles\tmobile_element\t$insertionqstart\t$insertionqend\t$evalue\t$strand\t.\tID=mobile_element_$counter;Name=$isname;rpt_family=$isfamily;Note=aln:$qstart..$qend,scov:$scov,pident:$pident,support:$insertionsupport" >> $insertionannotdir/${i}_identified_insertions.gff
                ...
                Rscript cluster_identified_ins_del.R $insertionannotdir/${i}_identified_insertions.txt > /dev/null 2>&1
        done
...
if [[ ! -d $deletionannotdir ]] ; then
        ...
        for i in $barcodes ; do
                ...
                while IFS="	" read qseqid qstart qend sseqid sstart send score evalue pident ; do
                                ...
                                if [[ "$req1" == 1 ]] && [[ "$req2" == 1 ]] && [[ "$req3" == 1 ]]; then
                                        echo -e "$deletionacc\tsniffles\tmobile_element\t$deletionqstart\t$deletionqend\t$evalue\t$strand\t.\tID=mobile_element_$counter;Name=$isname;rpt_family=$isfamily;Note=aln:$qstart..$qend,scov:$scov,pident:$pident,support:$deletionsupport" >> $deletionannotdir/${i}_identified_deletions.gff
                ...
                Rscript cluster_identified_ins_del.R $deletionannotdir/${i}_identified_deletions.txt > /dev/null 2>&1
        done
